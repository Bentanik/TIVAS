...
export const createNewProject = (
  { name, description, location, buildingStatus, type, features, attractions },
  fileData
) => {
      ...
      if (pageInput <= countPages) {
        ...
        if (response && response.length !== 0) {
          for (let i = 0; i < response.length; i++) {
            const locationDB = await db.Location.findByPk(
              response[i].locationID
            );
      ...
      resolve({
        ...
        message:
          pageInput > countPages
            ...
            ? `Get all of projects results`
            : "Can not find any projects!",
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve({
        ...
        message:
          response.length !== 0 ? "All Locations" : "Can not find any Location",
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const projectResponse = await db.Project.findAll({
        ...
        having: type
          ? literal(`COUNT(TypeOfProjects.projectID) = ${type.length}`)
          : literal(`COUNT(TypeOfProjects.projectID) > 0`),
        subQuery: false,
      });
      let countPages = projectResponse.length !== 0 ? 1 : 0;
      if (projectResponse.length / queries.limit > 1) {
...
export const searchNameAndLocationProject = (info, limit) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (
        bestMatch.length !== 0 &&
        projectByNameResponse.length !== 0 &&
        projectByLocationResponse.length !== 0
      ) {
        response = {};
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve({
        ...
        mess:
          response && response.length !== 0
            ? "Get top 10 new projects results"
            : "Can not find any Projects!",
        data: response && response.length !== 0 ? response : null,
        count: response ? response.length : 0,
      });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const projectResponse = await db.Project.findByPk(id, {
        attributes: {
          exclude: ["createdAt", "updatedAt", "thumbnailPathName"],
        },
        nest: true,
        //raw: true,
        include: [
      ...
      if (projectResponse) {
        ...
        for (let i = 0; i < projectResponse.TypeOfProjects.length; i++) {
          for (
            let j = 0;
            j < projectResponse.TypeOfProjects[i].TypeRooms.length;
            j++
          ) {
            response.TypeRoom.push({
              ...
              bathrooms:
                projectResponse.TypeOfProjects[i].TypeRooms[j].bathrooms,
              persons: projectResponse.TypeOfProjects[i].TypeRooms[j].persons,
              bedTypes:
                projectResponse.TypeOfProjects[i].TypeRooms[j].bedTypes.split(
                  ","
                ),
              amenities:
                projectResponse.TypeOfProjects[i].TypeRooms[j].amenities?.split(
                  ","
                ),
...
export const updateBooking = ({ openDate, closeDate }, id) => {
  return new Promise(async (resolve, reject) => {
    try {
      const errorMessage = [];
      ...
      if (
        project.status == 1 ||
        (project.status == 0 && openDate < closeDate)
      ) {
        if (openDate > closeDate || openDate == closeDate) {
          errorMessage.push(
            "Can not change openDate because new date is less than old openDate"
          );
        } else {
        ...
        errorMessage.push(
          "Can not set booking time because this project in booking time"
        );
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      resolve({
        ...
        message: response.reservationDate
          ? `Project (${id})'s reservation info.`
          : `Project(${id}) does not have any reservation info!`,
...
export const updateReservationInfo = (
  id,
  { reservationDate, reservationPrice, openDate, closeDate }
) => {
  return new Promise(async (resolve, reject) => {
    try {
      const projectResponse = await db.Project.findByPk(id);
      ...
      if (projectResponse) {
        ...
        if (
          (projectResponse.openDate?.getTime() !==
            convertDate(openDate).getTime() ||
            projectResponse.closeDate?.getTime() !==
              convertDate(closeDate).getTime()) &&
          ...
          for (let i = 0; i < count1; i++) {
            const user1 = await db.User.findByPk(
      ...
      resolve({
        err:
          !(
            projectResponse?.status === 1 &&
            (projectResponse.reservationDate?.getTime() !==
              convertDate(reservationDate).getTime() ||
              projectResponse?.reservationPrice !== reservationPrice)
          ) &&
          !(
        ...
        if (projectResponse.length !== 0) {
          for (let i = 0; i < projectResponse.length; i++) {
            ...
            for (let j = 0; j < projectResponse[i].TypeOfProjects.length; j++) {
              project.typeOfProject.push(
                projectResponse[i].TypeOfProjects[j].Type.name
              );
            }
