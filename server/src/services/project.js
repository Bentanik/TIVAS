    ...
    try {
      ...
      if (projectResponse) {
        ...
        if (response.Project.attractions) {
          response.Project.attractions =
            response.Project.attractions.split(",");
        }
        if (typeRoomResponse) {
          response.typeRooms = typeRoomResponse;
          for (let i = 0; i < response.typeRooms.length; i++) {
            if (response.typeRooms[i].bedTypes) {
              response.typeRooms[i].bedTypes =
                response.typeRooms[i].bedTypes.split(",");
            }
            if (response.typeRooms[i].amenities) {
              response.typeRooms[i].amenities =
                response.typeRooms[i].amenities.split(",");
            }
          }
        }
      }
